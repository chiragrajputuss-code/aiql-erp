...
const OFF_TOPIC_MESSAGE = "This doesn't look like a financial query. I can only answer questions about your " +
...
async function classifyWithLLM(question) {
    ...
    if (!apiKey)
        return { pass: true }; // no key → fail open, let pipeline decide
    try {
        ...
        if (!res.ok)
            return { pass: true }; // API error → fail open
