...
describe("createConnector — ZOHO_BOOKS", () => {
    ...
    it("passes onTokenRefresh callback", async () => {
        ...
        const c = createConnector("ZOHO_BOOKS", { accessToken: "old", refreshToken: "ref", clientId: "id", clientSecret: "sec",
            organisationId: "org", tokenExpiresAt: new Date(0) }, { onTokenRefresh: cb });
...
describe("token refresh", () => {
    ...
    it("isTokenExpired() returns false for future expiry (> 5min)", () => {
        const c = makeConnector({ tokenExpiresAt: new Date(Date.now() + 3600000) });
    ...
    it("auto-refreshes token before API call when expired", async () => {
        ...
        mockGet.mockResolvedValueOnce(ORG_RESPONSE);
        await c.testConnection();
        expect(mockPost).toHaveBeenCalledWith(expect.stringContaining("/oauth/v2/token"), expect.anything(), expect.anything());
    });
...
describe("getEntityLists()", () => {
    ...
    it("vendors and customers are separate (no overlap)", async () => {
        ...
        for (const v of vendors)
            expect(customers).not.toContain(v);
    });
...
describe("executeQuery()", () => {
    ...
    it("accepts JSON query format with params", async () => {
        ...
        expect(mockGet).toHaveBeenCalledWith(expect.stringContaining("/profitandloss"), expect.objectContaining({ params: expect.objectContaining({ from_date: "2026-04-01" }) }));
    });
