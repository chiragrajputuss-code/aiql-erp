...
import { mapColumn } from "../../file-upload/column-mapper";
...
function makeExcel(headers, rows) {
...
// ─── Validator ────────────────────────────────────────────────────────────────
describe("validator", () => {
    function resolvedFrom(pairs) {
        return pairs.map(([original, canonical, dropped]) => ({
            originalName: original, canonicalName: canonical,
            confidence: 1.0, detectionMethod: "exact", dropped,
        }));
    }
...
describe("full pipeline — 15-column GL Excel → 8 canonical", () => {
    ...
    it("parses the Excel buffer correctly", () => {
        const buf = makeExcel(GL_HEADERS, SAMPLE_ROWS.map((r) => GL_HEADERS.map((h) => r[h])));
    ...
    it("produces exactly 8 active canonical columns after full pipeline", () => {
        const sampleValues = (header) => SAMPLE_ROWS.map((r) => r[header] ?? "");
        // Step 1: Skip
        ...
        // Step 2: Map
        const mappings = nonSkipped.map((h) => mapColumn(h, sampleValues(h)));
