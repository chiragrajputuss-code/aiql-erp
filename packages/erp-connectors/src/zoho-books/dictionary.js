 ...
 */
export async function getZohoEntityLists(connector) {
    ...
    let hasMore = true;
    while (hasMore) {
        const data = await connector.get("/contacts", {
            status: "active",
        ...
        for (const c of contacts) {
            ...
            if (c.contact_type === "vendor")
                vendors.push(c.contact_name.trim());
            if (c.contact_type === "customer")
                customers.push(c.contact_name.trim());
        }
