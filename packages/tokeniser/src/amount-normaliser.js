...
function parseEuropean(s) {
    // European: periods = thousands, comma = decimal  →  swap then parse
    return parseFloat(s.replace(/\./g, "").replace(",", "."));
}
function collectMatches(text, re, parser, used) {
    const results = [];
    re.lastIndex = 0;
    let m;
    while ((m = re.exec(text)) !== null) {
        const pos = { start: m.index, end: m.index + m[0].length };
        if (used.some((u) => overlaps(u, pos)))
            continue;
...
const RE_KEYWORD_BEFORE = /\b(?:amount|balance|total|revenue|expense|cost|invoice|payment|over|under|above|below|exceeds|greater|less|baaki|raashi)\s+(?:of\s+)?(\d{1,3}(?:,\d{3})+|\d{4,})(?:\.\d+)?\b/gi;
// 12. Plain numbers > 1000 adjacent to financial keywords (number THEN keyword)
const RE_KEYWORD_AFTER = /\b(\d{1,3}(?:,\d{3})+|\d{4,})(?:\.\d+)?\s+(?:amount|balance|total|rupees?)\b/gi;
// ─── Main export ──────────────────────────────────────────────────────────────
