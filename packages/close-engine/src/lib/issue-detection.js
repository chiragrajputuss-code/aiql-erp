 ...
 */
export function buildVoucherImbalanceIssue(rows) {
    if (rows.length === 0)
        return null;
    ...
    return {
        ...
        description: `Each voucher must have equal debits and credits. ` +
...
export function buildDuplicateIssue(rows) {
    if (rows.length === 0)
        return null;
...
export const QUESTION_MATERIALITY_INR = 10000;
export const QUESTION_BUDGET_MAX = 3;
export const FLUX_MATERIAL_ABS_THRESHOLD = 50000;
export const FLUX_MATERIAL_PCT_THRESHOLD = 10;
export function isMaterialFlux(variance, priorBalance) {
    const absVar = Math.abs(variance);
    if (absVar < FLUX_MATERIAL_ABS_THRESHOLD)
        return false;
    if (priorBalance === 0)
        return absVar >= FLUX_MATERIAL_ABS_THRESHOLD;
...
export const PROVIDER_RATES_USD_PER_M = {
...
export function estimateCostInr(providerKey, inputTokens, outputTokens) {
    ...
    const usd = (inputTokens / 1000000) * rate.input + (outputTokens / 1000000) * rate.output;
