...
function assertReadOnly(query) {
    for (const re of WRITE_PATTERNS) {
        if (re.test(query)) {
            throw new Error("Write operations (IMPORTDATA / INSERT / UPDATE / DELETE) are not permitted on ERP connections");
        }
    }
}
function isRawXml(query) {
 ...
 */
export async function executeTallyQuery(connector, query) {
