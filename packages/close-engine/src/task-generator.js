...
import { sanitiseWatchAccount } from "./intent-parser";
export async function prepareCloseContext(connectionId, startDate, endDate) {
 ...
 */
function customWatchRecon(name) {
 ...
 */
function customWatchPartyRecon(partyFragment) {
...
// ─── Main entry point ────────────────────────────────────────────────────────
export async function generateAdaptiveTemplate(connectionId, startDate, endDate, options = {}) {
    ...
    const isExcluded = (key) => intent?.exclusions?.includes(key) ?? false;
    // Helper: emphasised area? (used to bump sortOrder priority for ADAPTIVE)
    const isFocused = (area) => intent?.focusAreas?.includes(area) ?? false;
    ...
    for (const issue of actionableIssues) {
        ...
        reasoning.push(`Anomaly (${issue.severity}): ${issueTaskTitle(issue)}` +
            (issue.exposure ? ` — ₹${issue.exposure.toLocaleString("en-IN", { maximumFractionDigits: 0 })} exposure` : ""));
    }
    // ── Account-driven: Reconciliations ─────────────────────────────────────
    const reconDeps = ["opening-balance"];
    for (const t of tasks.filter((tk) => tk.category === "CUSTOM"))
        reconDeps.push(t.key);
    ...
    const includeBank = accounts.bank.length > 0 &&
    ...
    if (intent && (intent.riskFlags.length > 0 || intent.oneOffEvents.length > 0)) {
        ...
        if (intent.riskFlags.length > 0)
            items.push(...intent.riskFlags.map((f) => `Risk: ${f}`));
        if (intent.oneOffEvents.length > 0)
            items.push(...intent.oneOffEvents.map((e) => `One-off: ${e}`));
        tasks.push({
    ...
    if (reviewDeps.length === 0)
        reviewDeps.push("opening-balance");
    ...
    // ── Sign-off ────────────────────────────────────────────────────────────
    const signoffDeps = ["pl-review", "bs-review", "flux-analysis"].filter((k) => tasks.some((t) => t.key === k));
    tasks.push({
        key: "cfo-signoff",
