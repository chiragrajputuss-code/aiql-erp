 ...
 */
export async function getUploadEntityLists(tableName) {
    if (!tableName.startsWith("upload_"))
        return { vendors: [], customers: [], employees: [] };
    // Discover available columns
    const colRows = await prisma.$queryRawUnsafe(`SELECT column_name FROM information_schema.columns WHERE table_name = $1 AND table_schema = 'public'`, tableName);
    const cols = new Set(colRows.map((r) => r.column_name));
    const vendors = [];
    const customers = [];
    // Strategy 1: explicit vendor_name / customer_name columns
    if (cols.has("vendor_name")) {
        const rows = await prisma.$queryRawUnsafe(`SELECT DISTINCT vendor_name FROM "${tableName}" WHERE vendor_name IS NOT NULL AND vendor_name <> ''`);
        vendors.push(...rows.map((r) => r.vendor_name));
    }
    if (cols.has("customer_name")) {
        const rows = await prisma.$queryRawUnsafe(`SELECT DISTINCT customer_name FROM "${tableName}" WHERE customer_name IS NOT NULL AND customer_name <> ''`);
    ...
    if (vendors.length === 0 && customers.length === 0 && cols.has("account_group") && cols.has("account_name")) {
        const rows = await prisma.$queryRawUnsafe(`SELECT DISTINCT account_name, account_group FROM "${tableName}" WHERE account_name IS NOT NULL`);
        for (const row of rows) {
            const group = (row.account_group ?? "").toLowerCase();
            if (VENDOR_GROUPS.has(group))
                vendors.push(row.account_name);
            else if (CUSTOMER_GROUPS.has(group))
                customers.push(row.account_name);
        }
    }
    // Strategy 3: party_name as vendor fallback
    if (vendors.length === 0 && customers.length === 0 && cols.has("party_name")) {
        const rows = await prisma.$queryRawUnsafe(`SELECT DISTINCT party_name FROM "${tableName}" WHERE party_name IS NOT NULL AND party_name <> ''`);
