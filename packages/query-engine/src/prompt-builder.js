import { preprocessHinglish, tokenise } from "@aiql/tokeniser";
...
function shouldIncludeColumn(col) {
    if (col.isPrimaryKey || col.isForeignKey)
        return true;
    if (col.isAmount || col.isDate || col.isName)
        return true;
    if (PRIORITY_CATEGORIES.has(col.dataType))
        return true;
    return false;
}
...
function formatRelationLine(rel) {
...
// ─── System prompt ────────────────────────────────────────────────────────────
function buildSystemPrompt(schema, erpType, sqlDialect, dateContext) {
 ...
 */
function preprocessQuery(rawQuestion, config, dictionary) {
...
export function buildPrompt(options) {
    const { schema, rawQuestion, erpType, sqlDialect = "postgresql", config = {}, dictionary, fewShotExamples = [], dateContext, conversationContext = [], } = options;
    const { preprocessed: _preprocessed, tokenised, tokenMap } = preprocessQuery(rawQuestion, config, dictionary);
