...
function calcCost(model, tokensIn, tokensOut) {
    ...
    return (tokensIn * rates.input + tokensOut * rates.output) / 1000000;
}
...
export function assessComplexity(question) {
    let score = 0;
    if (WINDOW_FN_RE.test(question))
        score += 4;
    if (SUBQUERY_RE.test(question))
        score += 3;
    score += (question.match(JOIN_RE) ?? []).length;
    if (AGGREGATE_RE.test(question))
        score += 1;
    if (score >= 4)
        return "complex";
    if (score >= 1)
...
// ─── Custom org provider ──────────────────────────────────────────────────────
function buildCustomProvider(config) {
    if (!config.llmProvider || config.llmProvider === "AIQL_MANAGED")
        return null;
...
// ─── Individual callers ───────────────────────────────────────────────────────
async function callGroq(systemPrompt, userPrompt, retried) {
...
async function callOpenAI(systemPrompt, userPrompt, model, retried) {
...
async function callClaude(systemPrompt, userPrompt, retried) {
...
// ─── Thresholds ───────────────────────────────────────────────────────────────
const CONFIDENCE_RETRY_THRESHOLD = parseFloat(process.env.AIQL_CONFIDENCE_RETRY_THRESHOLD ?? "0.75");
// ─── Router ───────────────────────────────────────────────────────────────────
//
...
//
export async function routeQuery(systemPrompt, userPrompt, orgConfig = { llmProvider: null, llmModel: null, llmApiKey: null }) {
    ...
    if (!available.groq && !available.openai && !available.claude) {
        throw new Error("No LLM API key configured. Set GROQ_API_KEY (free at console.groq.com), OPENAI_API_KEY, or ANTHROPIC_API_KEY in your .env");
    }
    // 2. Complex queries skip Groq — go straight to OpenAI mini or Claude
    if (complexity === "complex") {
        if (available.openai)
            return callOpenAI(systemPrompt, userPrompt, OPENAI_MINI_MODEL, false);
        if (available.claude)
            return callClaude(systemPrompt, userPrompt, false);
    }
