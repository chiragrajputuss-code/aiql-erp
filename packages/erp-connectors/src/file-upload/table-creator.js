...
export function buildTableName(orgId, fileId) {
 ...
 */
export async function createTempTable(orgId, fileId, mappings, rows) {
    ...
    const canonicalNames = activeCols.map((m) => m.canonicalName);
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
        ...
        await prisma.$executeRawUnsafe(`INSERT INTO "${tableName}" (${colList}) VALUES ${placeholders.join(",")}`, ...values);
    }
    // ── Indexes ───────────────────────────────────────────────────────────────
    if (canonicalNames.includes("transaction_date")) {
        await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "${tableName}_date_idx" ON "${tableName}" ("transaction_date")`);
    }
    if (canonicalNames.includes("account_name")) {
        await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "${tableName}_acct_idx" ON "${tableName}" ("account_name")`);
    }
    return tableName;
}
export async function dropTempTable(tableName) {
...
export async function listOrgTables(orgId) {
    ...
    const rows = await prisma.$queryRawUnsafe(`SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename LIKE $1`, `${prefix}%`);
    return rows.map((r) => r.tablename);
}
// ─── Value normalisation ──────────────────────────────────────────────────────
function normaliseValue(raw, canonicalName) {
