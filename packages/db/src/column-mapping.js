 ...
 */
export async function upsertOrgMappings(orgId, mappings) {
    await Promise.all(mappings.map((m) => prisma.orgColumnMapping.upsert({
        where: {
 ...
 */
export async function getOrgMappings(orgId) {
 ...
 */
export async function clearOrgMappings(orgId) {
