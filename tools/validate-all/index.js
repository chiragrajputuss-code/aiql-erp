...
import { parseCsv, mapColumn, resolveRedundancy, createTempTable, buildUploadSchema, getUploadEntityLists, } from "@aiql/erp-connectors";
import { seedDefaultPinnedQueries } from "@aiql/db";
import { runDataQualityScan, generateAdaptiveTemplate, parseUserIntent, patternKeyForScanIssue, } from "@aiql/close-engine";
...
import { buildColMap, applyColMap, getTableColumns, makeSqlDefensive, } from "../../packages/close-engine/src/utils/column-mapping";
...
const COMPANY_INTENTS = {
    "kumar_textiles.csv": "GST notice received — check Ganesh Traders Pvt Ltd and Shree Services LLP ITC entries carefully. Also verify if IGST applied correctly on export sales.",
    "buildpro.csv": "Bank loan renewal — need project cost breakdown and verify subcontractor invoices carefully, creditor entries may be missing.",
    "spice_garden.csv": "Owner suspects cash leakage. Check all journal entries above 1 lakh, especially Diwali Bonus to Staff entries.",
    "sharma_electronics.csv": "Statutory audit prep. Need debtor ageing, related party transactions (check Sharma family parties), and top creditor balances.",
    "techvista.csv": "Investor due diligence. Check EEFC account for compliance — no rupee expenses should be routed through ICICI EEFC USD A/c.",
    "patel_distributors.csv": "Focus on AP — Patel Distributors has 1380 vouchers and 123 parties. Need to verify creditor balances and GST input credits.",
    "steelco.csv": "Steel trader has 110 lakh in unbalanced vouchers. Drill down on specific subcontractor and material purchase patterns.",
    "learnright.csv": "EdTech with 102 lakh dr-cr gap. Check fee receipts vs revenue recognition alignment.",
    "apollo_diag.csv": "Diagnostics chain with 240 parties. Focus on party reconciliation and unmapped accounts.",
    "speedy_cargo.csv": "Logistics company. Standard close — focus on party reconciliation for 77 carriers/clients.",
};
 ...
 */
async function seedKnowledgeAndApply(connectionId, scan) {
...
async function cleanupAll() {
    ...
    const tables = await prisma.$queryRawUnsafe(`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename LIKE 'upload_validate_test_%'`);
    ...
    if (kbDeleted.count > 0)
        console.error(`  deleted ${kbDeleted.count} knowledge entries`);
...
async function ingestCompany(file) {
    ...
    const columnMappingJson = JSON.stringify(resolved
    ...
    const distinctAccounts = await prisma.$queryRawUnsafe(`SELECT DISTINCT account_name FROM "${tableName}" WHERE account_name IS NOT NULL AND account_name <> ''`);
 ...
 */
async function executeRecon(template, connectionId, tableName, start, end) {
    const startIso = start.toISOString().slice(0, 10);
    const endIso = end.toISOString().slice(0, 10);
    // 1. Substitute placeholders that the template generator left in
    const substitute = (q) => q.replace(/\{tableName\}/g, tableName)
    ...
    const finalize = (q) => makeSqlDefensive(applyColMap(substitute(q), colMap), presentCols);
    const params = template.params ?? [];
    const sourceSql = finalize(template.sourceQuery);
    const targetSql = finalize(template.targetQuery);
    const sourceRows = await prisma.$queryRawUnsafe(sourceSql, ...params);
    const targetRows = await prisma.$queryRawUnsafe(targetSql, ...params);
...
async function runOneCompany(file) {
    ...
    try {
        ...
        result.ingestion.error = err.message;
        result.durationMs = Date.now() - t0;
        return result;
    }
    // Determine the close period from actual data
    const dateRows = await prisma.$queryRawUnsafe(`SELECT transaction_date FROM "${result.ingestion.tableName}" WHERE transaction_date IS NOT NULL`);
    ...
    try {
        const { template, reasoning } = await generateAdaptiveTemplate(connectionId, start, end, { profile: "STANDARD" });
    ...
    let adaptiveTemplate = null;
    try {
        const { template, reasoning } = await generateAdaptiveTemplate(connectionId, start, end, { profile: "ADAPTIVE", intent });
    ...
    if (adaptiveTemplate && result.ingestion.tableName) {
        ...
        for (const task of adaptiveTemplate.tasks) {
            if (!task.reconciliation)
                continue;
            try {
                const r = await executeRecon(task.reconciliation, connectionId, result.ingestion.tableName, start, end);
...
function renderReport(results) {
    ...
    for (const r of results) {
        ...
        const recOk = r.reconciliations
            ...
            : "—";
        out.push(`| ${r.file.replace(".csv", "")} | ${r.ingestion.rowCount} | ${r.ingestion.columnsMapped}/${r.ingestion.headerCount} | ${scanIcon} | ${r.scan?.totalIssues ?? 0} | ${formatRupees(r.scan?.totalExposure ?? 0)} | ${taskS} | ${taskA} | ${party} | ${recOk} |`);
    }
    out.push("");
    for (const r of results) {
        out.push(`---\n\n## ${r.file}\n`);
        ...
        if (r.intentParse) {
            ...
            if (!r.intentParse.success) {
                ...
                if (r.intentParse.focusAreas.length)
                    out.push(`- Focus areas: ${r.intentParse.focusAreas.join(", ")}`);
                if (r.intentParse.watchAccounts.length)
                    out.push(`- Watch accounts: ${r.intentParse.watchAccounts.join(", ")}`);
                if (r.intentParse.watchParties.length)
                    out.push(`- **Watch parties: ${r.intentParse.watchParties.join(", ")}**`);
                if (r.intentParse.riskFlags.length)
                    out.push(`- Risk flags: ${r.intentParse.riskFlags.join(", ")}`);
                out.push("");
            }
        }
        if (r.taskGenStandard) {
            ...
            if (!r.taskGenStandard.success) {
                ...
                for (const t of r.taskGenStandard.taskTitles)
                    out.push(`- ${t}`);
                out.push("");
            }
        }
        if (r.taskGenAdaptive) {
            ...
            if (!r.taskGenAdaptive.success) {
                ...
                for (const t of r.taskGenAdaptive.taskTitles)
                    out.push(`- ${t}`);
                out.push("");
            }
        }
...
async function main() {
    ...
    for (const file of SAMPLE_FILES) {
        ...
        console.error(`   ingest=${r.ingestion.success ? "✓" : "✗"} scan=${scanIcon} ${taskS} ${taskA} ` +
