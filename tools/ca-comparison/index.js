...
function analyze(name, csvBuffer) {
    ...
    if (cols.date) {
        ...
        if (dates.length > 20) {
            ...
            for (const t of dates) {
                if (t < p5 - 30 * 86400000 || t > p95 + 30 * 86400000)
                    a.dateOutlierCount++;
            }
        }
    }
    ...
    for (const row of rows) {
        ...
        if (!acc.includes("bank") && !acc.includes("cash"))
            continue;
        const dr = toNum(row[cols.debit]);
        const cr = toNum(row[cols.credit]);
    ...
    for (const [, bal] of bankBalance) {
        if (bal < -1)
            a.signAnomalyCount++; // negative bank without OD context
    }
    ...
    if (/bank|cash/.test(accountString))
        a.reconCategories++;
    if (/creditor|payable/.test(accountString))
        a.reconCategories++;
    if (/debtor|receivable/.test(accountString))
        a.reconCategories++;
    if (/gst|cgst|sgst|igst/.test(accountString))
        a.reconCategories++;
    if (/stock|inventory/.test(accountString))
        a.reconCategories++;
...
// ─── Time estimates ──────────────────────────────────────────────────────────
function manualMinutes(a) {
    return (MANUAL_MINUTES.fixedOverheadCloseMinutes +
...
function aiqlMinutes(a) {
    ...
    return (AIQL_MINUTES.uploadAndClassify +
...
function renderMarkdown(reports) {
    ...
    for (const r of reports) {
        lines.push(`| ${r.analysis.name.padEnd(22)} | ${r.analysis.totalRows} | ${r.analysis.totalVouchers} | ${r.analysis.totalAnomalies} | ${fmtMin(r.manualMin)} | ${fmtMin(r.aiqlMin)} | ${fmtMin(r.savedMin)} | ${r.savedPct}% | ${fmtMin(r.monthTwoSavedMin)} |`);
    }
    lines.push("");
    // ─── Detail per company ───
    ...
    for (const r of reports) {
        ...
        lines.push("");
        if (a.voucherImbalanceCount > 0)
            lines.push(`- ${a.voucherImbalanceCount} voucher${a.voucherImbalanceCount > 1 ? "s" : ""} where Dr ≠ Cr (₹${Math.round(a.voucherImbalanceExposure).toLocaleString("en-IN")} exposure) — manual time: ~${Math.round(a.voucherImbalanceCount * MANUAL_MINUTES.perVoucherImbalance)} min`);
        if (a.duplicateCount > 0)
            lines.push(`- ${a.duplicateCount} likely duplicate transaction${a.duplicateCount > 1 ? "s" : ""} — manual time: ~${Math.round(a.duplicateCount * MANUAL_MINUTES.perDuplicateCheck)} min`);
        if (a.dateOutlierCount > 0)
            lines.push(`- ${a.dateOutlierCount} entries dated outside the typical period — manual time: ~${Math.round(a.dateOutlierCount * MANUAL_MINUTES.perDateOutlier)} min`);
        if (a.gstMismatchCount > 0)
            lines.push(`- ${a.gstMismatchCount} voucher${a.gstMismatchCount > 1 ? "s" : ""} with CGST ≠ SGST — manual time: ~${Math.round(a.gstMismatchCount * MANUAL_MINUTES.perGstMismatch)} min`);
        if (a.signAnomalyCount > 0)
            lines.push(`- ${a.signAnomalyCount} bank/cash account${a.signAnomalyCount > 1 ? "s" : ""} with negative balance — manual time: ~${Math.round(a.signAnomalyCount * MANUAL_MINUTES.perSignAnomaly)} min`);
        if (a.missingFieldCount > 0)
            lines.push(`- ${a.missingFieldCount} rows with missing critical fields — manual time: ~${Math.round(a.missingFieldCount * MANUAL_MINUTES.perMissingFieldRow)} min`);
        if (a.unclassifiedAccountCount > 0)
            lines.push(`- ${a.unclassifiedAccountCount} unmapped account${a.unclassifiedAccountCount > 1 ? "s" : ""} (AIQL classifies + asks user to confirm) — manual time: ~${Math.round(a.unclassifiedAccountCount * MANUAL_MINUTES.perAccountClassify)} min`);
