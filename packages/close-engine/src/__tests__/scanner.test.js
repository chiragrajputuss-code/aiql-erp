...
vi.mock("../utils/column-mapping", () => ({
    ...
    applyColMap: (sql) => sql,
    getTableColumns: vi.fn().mockResolvedValue(new Set(["transaction_date", "account_name", "debit_amount", "credit_amount",
        "reference_number", "voucher_type", "party_name", "description"])),
    makeSqlDefensive: (sql) => sql,
...
describe("scanner — end-to-end ScanResult", () => {
    it("aggregates totalIssues, bySeverity, and totalExposure correctly", async () => {
        ...
        if (result.issues.length >= 2) {
            const sevRank = { critical: 0, review: 1, info: 2 };
    ...
    it("throws when table name not found", async () => {
        ...
        cm.getTableName.mockResolvedValueOnce(null);
...
describe("scanner — TDS deduction check", () => {
    ...
    it("skips the check if reference_number column is missing", async () => {
        const cm = await import("../utils/column-mapping");
        cm.getTableColumns.mockResolvedValueOnce(new Set(["transaction_date", "account_name", "debit_amount", "credit_amount", "party_name"])
...
describe("scanner — graceful degradation on SQL errors", () => {
    it("does not crash when a single check throws", async () => {
        ...
        mockQueryRawUnsafe.mockImplementation(async (sql) => {
            callCount++;
            if (callCount === 1)
                throw new Error("SQL error in first check");
            if (sql.includes("missing_date"))
                return [{ missing_date: 5, missing_account: 0, zero_amount: 0, both_dr_cr: 0 }];
            return [];
        });
