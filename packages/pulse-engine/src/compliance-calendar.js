// ─── Date helpers ─────────────────────────────────────────────────────────────
function daysBefore(target, today) {
    return Math.ceil((target.getTime() - today.getTime()) / 86400000);
}
function daysAfter(target, today) {
    return Math.floor((today.getTime() - target.getTime()) / 86400000);
}
function isInAlertWindow(dueDate, today, leadDays, graceDays) {
...
// ─── Main generator ───────────────────────────────────────────────────────────
export function generateComplianceAlerts(today, ctx) {
    const alerts = [];
    // Historical workspaces don't need compliance deadline alerts
    if (ctx.dataIntent === "HISTORICAL")
        return alerts;
