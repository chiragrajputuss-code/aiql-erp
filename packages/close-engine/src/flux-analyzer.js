...
const MATERIAL_ABS_THRESHOLD = 50000; // ₹50K
const MATERIAL_PCT_THRESHOLD = 10; // 10%
function isMaterial(variance, priorBalance) {
    const absVar = Math.abs(variance);
    if (absVar < MATERIAL_ABS_THRESHOLD)
        return false;
    // If prior was 0, any current activity over ₹50K is material
    if (priorBalance === 0)
        return absVar >= MATERIAL_ABS_THRESHOLD;
    const pct = (absVar / Math.abs(priorBalance)) * 100;
    return pct >= MATERIAL_PCT_THRESHOLD;
}
// ─── SQL helper ───────────────────────────────────────────────────────────────
async function runSql(sql, colMap) {
...
async function fetchAccountActivity(table, start, end, colMap) {
    const sql = `
    SELECT account_name,
  ...
  `;
    const rows = await runSql(sql, colMap);
    const map = new Map();
    for (const r of rows)
        map.set(r.account_name, toNum(r.net));
    return map;
}
...
async function analyzeFluxItem(accountName, accountType, currentBalance, priorBalance, variance, variancePct) {
    ...
    if (!parsed.summary || !Array.isArray(parsed.causes) || !Array.isArray(parsed.actions))
        return null;
    return {
        pattern: parsed.pattern ?? "unknown",
        summary: parsed.summary,
        causes: parsed.causes.filter((c) => typeof c === "string"),
        actions: parsed.actions.filter((a) => typeof a === "string"),
        confidence: typeof parsed.confidence === "number" ? Math.max(0, Math.min(1, parsed.confidence)) : 0.5,
    };
}
// ─── Main entry point ────────────────────────────────────────────────────────
export async function runFluxAnalysis(connectionId, startDate, endDate, options) {
    const t0 = Date.now();
    ...
    const allAccounts = new Set([...currentActivity.keys(), ...priorActivity.keys()]);
    const changes = [];
    ...
    for (const accountName of allAccounts) {
        ...
        if (Math.abs(currentBalance) < 1 && Math.abs(priorBalance) < 1)
            continue;
    ...
    if (withAI) {
        ...
        const analyses = await Promise.all(materialChanges.map((c) => analyzeFluxItem(c.accountName, c.accountType, c.currentBalance, c.priorBalance, c.variance, c.variancePct)));
 ...
 */
export async function runFluxForTask(taskId) {
    const task = await prisma.closeTask.findUniqueOrThrow({
        where: { id: taskId },
        include: { period: true },
    });
    const result = await runFluxAnalysis(task.period.connectionId, task.period.startDate, task.period.endDate, { withAI: true, maxAIAnalyses: 10 });
...
export async function getFluxRunForTask(taskId) {
    const saved = await prisma.fluxAnalysisRun.findUnique({ where: { taskId } });
    if (!saved)
        return null;
    let result;
    try {
        result = JSON.parse(saved.resultJson);
    }
    catch {
        return null;
    }
