...
import { classifyByName, classifyAccountNames, fillUnknownsByName, } from "../account-classifier";
...
describe("fillUnknownsByName", () => {
    it("only updates UNKNOWN entries", () => {
        const input = {
            "HDFC Bank A/c": "BANK", // already classified, leave alone
            "Sundry Creditors": "UNKNOWN", // fill in
        };
    ...
    it("leaves UNKNOWNs as UNKNOWN if pattern does not match", () => {
        const input = { "AC1001": "UNKNOWN" };
        const result = fillUnknownsByName(input);
        expect(result["AC1001"]).toBe("UNKNOWN");
    });
    it("does not mutate input", () => {
        const input = { "HDFC Bank A/c": "UNKNOWN" };
