...
describe("parseUserIntent — LLM path with mock fetch", () => {
    ...
    it("uses LLM output when heuristic confidence is low", async () => {
        ...
        const r = await parseUserIntent("we paid a one-time bonus in March that was significantly larger than usual, please make sure salary and bank balances reflect this correctly");
    ...
    it("extracts watchParties from LLM output and applies SQL-safety gate", async () => {
        ...
        const r = await parseUserIntent("GST notice received — check Ganesh Traders and Shree Services LLP ITC entries carefully, there may be a GSTR-2B mismatch");
        expect(r.source).toBe("llm");
        expect(r.watchParties).toContain("Ganesh Traders");
        expect(r.watchParties).toContain("Shree Services LLP");
