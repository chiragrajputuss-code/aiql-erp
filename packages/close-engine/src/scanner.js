...
// ─── SQL helper ───────────────────────────────────────────────────────────────
async function runSql(sql, colMap, presentColumns = new Set()) {
...
async function checkTrialBalance(table, start, end, colMap, presentColumns = new Set()) {
    const sql = `
    SELECT
  ...
  `;
    let rows;
    try {
        rows = await runSql(sql, colMap, presentColumns);
    }
    catch {
        return null;
    }
    const r = rows[0];
    if (!r || toNum(r.imbalance) < 1)
        return null; // ₹1 tolerance for rounding
  ...
  `;
    let breakdown = [];
    try {
        breakdown = await runSql(breakdownSql, colMap, presentColumns);
    }
    ...
    return {
        ...
        description: `Total debits (₹${toNum(r.total_dr).toLocaleString("en-IN", { maximumFractionDigits: 0 })}) ` +
...
async function checkDuplicateTransactions(table, start, end, colMap, presentColumns = new Set()) {
  ...
  `;
    ...
    return {
        ...
        description: `Transactions with the same party, same amount, and dates within 7 days are likely duplicates. ` +
...
async function checkDateOutliers(table, start, end, colMap, presentColumns = new Set()) {
    const sql = `
  ...
  `;
    ...
    return {
        ...
        description: `Entries dated more than 60 days before period start or 30 days after period end. ` +
...
async function checkMissingFields(table, start, end, colMap, presentColumns = new Set()) {
  ...
  `;
    ...
    if (total === 0)
        return null;
    const parts = [];
    if (toNum(r.missing_date) > 0)
        parts.push(`${r.missing_date} entries with no transaction date`);
    if (toNum(r.zero_amount) > 0)
        parts.push(`${r.zero_amount} entries where both debit and credit are zero`);
    return {
        code: "missing_fields",
        severity: "critical",
        category: "Data Integrity",
...
async function checkUnclassifiedAccounts(table, start, end, colMap, typeMap, presentColumns = new Set()) {
    const sql = `
  ...
  `;
    ...
    return {
        ...
        description: `These accounts are not classified into a financial category (Bank, AP, AR, Tax, etc.). ` +
...
async function checkGSTMismatch(table, start, end, colMap, presentColumns = new Set()) {
  ...
  `;
    ...
    return {
        ...
        description: `For intra-state transactions, CGST and SGST must be equal. ` +
...
async function checkSignAnomalies(table, start, end, colMap, payableAccounts, receivableAccounts, presentColumns = new Set()) {
    if (payableAccounts.length === 0 && receivableAccounts.length === 0)
        return null;
    const issues = [];
    let totalExposure = 0;
    ...
    `;
        try {
            const rows = await runSql(sql, colMap, presentColumns);
            for (const r of rows) {
    ...
    return {
        ...
        description: `Accounts where the balance direction is opposite to what's expected for the account type. ` +
...
async function checkPeriodCompleteness(table, start, end, colMap, presentColumns = new Set()) {
  ...
  `;
    let activeDays;
    try {
        activeDays = await runSql(sql, colMap, presentColumns);
    }
    catch {
        return null;
    }
  ...
  `;
    let typeStats;
    try {
        typeStats = await runSql(typeSql, colMap, presentColumns);
    }
    catch {
        typeStats = [];
    }
    ...
    const missing = expected.filter((e) => !Array.from(vtypeNames).some((v) => v.includes(e)));
    if (missing.length >= 3) {
        concerns.push({
    ...
    return {
        code: "period_completeness",
        severity: concerns.some((c) => c.type === "missing_voucher_types") ? "critical" : "review",
        category: "Period Validity",
        title: `Period coverage looks ${concerns.length === 1 ? "off" : "incomplete"}`,
        description: `Detected ${concerns.length} signal${concerns.length > 1 ? "s" : ""} that the uploaded data may not fully cover the requested period. ` +
...
async function checkTdsDeduction(table, start, end, colMap, presentColumns = new Set()) {
  ...
  `;
    ...
    return {
        ...
        description: `Under Section 194 of the Income Tax Act, TDS must typically be deducted on vendor payments ` +
...
async function checkDebtorsAging(table, end, colMap, receivableAccounts, presentColumns = new Set()) {
  ...
  `;
    ...
    const cutoff60 = new Date(end);
    cutoff60.setDate(cutoff60.getDate() - 60);
    const cutoff90 = new Date(end);
    ...
    return {
        ...
        description: `${stale.length} receivable account${stale.length > 1 ? "s have" : " has"} an outstanding debit balance ` +
...
// ─── Main entry point ─────────────────────────────────────────────────────────
export async function runDataQualityScan(connectionId, startDate, endDate) {
    const t0 = Date.now();
