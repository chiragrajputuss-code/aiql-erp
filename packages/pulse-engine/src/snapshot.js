 ...
 */
export function computeSnapshotFromRows(rows) {
    if (rows.length === 0) {
    ...
    for (const row of rows) {
        ...
        if (matchesAny(name, CASH_BANK_PATTERNS)) {
            cashAndBank += net;
            hasCashBank = true;
        }
        if (matchesAny(name, RECEIVABLE_PATTERNS)) {
            receivables += net;
            hasReceiv = true;
        }
        if (matchesAny(name, PAYABLE_PATTERNS)) {
            payables += net;
            hasPay = true;
        }
    }
