...
function extractIdentifiers(sql) {
    ...
    while ((m = JOIN_TABLE_RE.exec(sql)) !== null)
        tables.add(m[1].toLowerCase());
...
// ─── Schema match scorer ──────────────────────────────────────────────────────
function scoreSchemaMatch(sql, schema) {
    if (!sql)
        return { score: 0, hallucinations: [] };
    ...
    const schemaColNames = new Set(schema.tables.flatMap((t) => t.columns.map((c) => c.name.toLowerCase())));
...
function scoreTemplateMatch(question, templates) {
    ...
    for (const tpl of templates) {
        ...
        if (hit)
            return 1.0; // known query pattern — high confidence
    }
 ...
 */
export function calculateConfidence(response, schema, question, templates = BUILT_IN_TEMPLATES) {
