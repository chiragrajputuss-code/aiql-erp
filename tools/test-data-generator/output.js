...
// ─── CSV writer ──────────────────────────────────────────────────────────────
export function writeCSV(outPath, company, transactions) {
    const cols = COLUMN_MAPS[company.columnConvention];
    // Build header row
    const headers = [cols.date, cols.voucherNo, cols.voucherType, cols.account];
    if (cols.party)
        headers.push(cols.party);
    if (cols.vendor)
        headers.push(cols.vendor);
    if (cols.customer)
        headers.push(cols.customer);
    ...
    for (const txn of transactions) {
        ...
        if (cols.party)
            row.push(txn.partyName ?? "");
        if (cols.vendor)
            row.push(txn.vendorName ?? "");
        if (cols.customer)
            row.push(txn.customerName ?? "");
...
// ─── README writer ───────────────────────────────────────────────────────────
export function writeREADME(outPath, company, transactions, issues) {
    const issuesByType = issues.reduce((acc, i) => {
        var _a;
        (acc[_a = i.type] ?? (acc[_a] = [])).push(i);
        return acc;
    }, {});
    ...
    if (cfg.voucherImbalance > 0)
        sections.push(`- ${cfg.voucherImbalance} voucher imbalance(s) — \`voucher_imbalance\``);
    if (cfg.duplicateTransactions > 0)
        sections.push(`- ${cfg.duplicateTransactions} duplicate transaction(s) — \`duplicate_transactions\``);
    if (cfg.dateOutliers > 0)
        sections.push(`- ${cfg.dateOutliers} date outlier(s) — \`date_outliers\``);
    if (cfg.missingFields > 0)
        sections.push(`- ${cfg.missingFields} missing field(s) — \`missing_fields\``);
    if (cfg.unclassifiedAccounts > 0)
        sections.push(`- ${cfg.unclassifiedAccounts} unclassified account(s) — \`unclassified_accounts\``);
    if (cfg.gstMismatch > 0)
        sections.push(`- ${cfg.gstMismatch} CGST ≠ SGST mismatch(es) — \`gst_mismatch\``);
    if (cfg.signAnomalies > 0)
        sections.push(`- ${cfg.signAnomalies} sign anomaly(ies) — \`sign_anomalies\``);
    sections.push("");
    sections.push("---");
    sections.push("");
