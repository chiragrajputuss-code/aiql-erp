 ...
 */
export async function safeLlmCall(req) {
    ...
    const audit = aggregateAudit(tk.auditLog);
    ...
    try {
        const res = await fetch(req.endpoint, {
            ...
            signal: AbortSignal.timeout(req.timeoutMs ?? 8000),
        });
        if (!res.ok)
            return null;
...
function aggregateAudit(entries) {
...
function sanitiseEndpointForLog(endpoint) {
