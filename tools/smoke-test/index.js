...
import { runDataQualityScan, runFluxAnalysis, computeReadinessScore, generateAdaptiveTemplate, createClosePeriodFromTemplate, runReconciliation, startPlReview, } from "@aiql/close-engine";
function parseArgs() {
...
const log = {
    step: (msg) => process.stdout.write(`  → ${msg.padEnd(60)} `),
    pass: (ms, detail) => console.log(`✓ ${ms}ms${detail ? ` (${detail})` : ""}`),
    fail: (ms, err) => console.log(`✗ ${ms}ms\n      ERROR: ${err}`),
    warn: (ms, msg) => console.log(`⚠ ${ms}ms — ${msg}`),
    section: (msg) => console.log(`\n━━━ ${msg} ━━━`),
};
// ─── Main per-company runner ─────────────────────────────────────────────────
async function runCompany(csvPath, keepData) {
    const id = path.basename(csvPath).replace(".csv", "");
    log.section(`${id}`);
    const report = { company: id, totalSteps: 0, passed: 0, failed: 0, warned: 0, steps: [] };
    const recordStep = (step, status, durationMs, detail, error) => {
    ...
    try {
        ...
        const setupRes = await timeStep("setup", async () => {
            const org = await prisma.organisation.create({
                data: {
                    ...
                    queriesResetAt: new Date(Date.now() + 30 * 86400000),
                },
            });
            return org.id;
        });
        if (!setupRes.ok) {
            log.fail(setupRes.ms, setupRes.error);
            recordStep("setup", "fail", setupRes.ms, undefined, setupRes.error);
            return report;
        }
        ...
        if (!readRes.ok) {
            log.fail(readRes.ms, readRes.error);
            recordStep("readCsv", "fail", readRes.ms, undefined, readRes.error);
            return report;
        }
        ...
        if (!mapRes.ok) {
            log.fail(mapRes.ms, mapRes.error);
            recordStep("mapColumns", "fail", mapRes.ms, undefined, mapRes.error);
            return report;
        }
        ...
        if (!tableRes.ok) {
            log.fail(tableRes.ms, tableRes.error);
            recordStep("createTable", "fail", tableRes.ms, undefined, tableRes.error);
            return report;
        }
        ...
        const connRes = await timeStep("createConnection", async () => {
            ...
            const sampleAccounts = await prisma.$queryRawUnsafe(`SELECT DISTINCT account_name FROM "${tableName}" WHERE account_name IS NOT NULL LIMIT 200`);
        ...
        if (!connRes.ok) {
            log.fail(connRes.ms, connRes.error);
            recordStep("createConnection", "fail", connRes.ms, undefined, connRes.error);
            return report;
        }
        ...
        const periodRes = await timeStep("detectPeriod", async () => {
            const range = await prisma.$queryRawUnsafe(`SELECT MIN(transaction_date) AS min_d, MAX(transaction_date) AS max_d FROM "${tableName}"`);
            if (!range[0] || !range[0].min_d)
                throw new Error("No transaction_date column or empty data");
            return { startDate: new Date(range[0].min_d), endDate: new Date(range[0].max_d) };
        });
        if (!periodRes.ok) {
            log.fail(periodRes.ms, periodRes.error);
            recordStep("detectPeriod", "fail", periodRes.ms, undefined, periodRes.error);
            return report;
        }
        ...
        if (!scanRes.ok) {
            log.fail(scanRes.ms, scanRes.error);
            recordStep("scan", "fail", scanRes.ms, undefined, scanRes.error);
        }
        else {
        ...
        const periodCreateRes = await timeStep("createPeriod", async () => {
            ...
            const period = await createClosePeriodFromTemplate(orgId, connectionId, [connectionId], `Smoke ${id}`, startDate, endDate, adaptive.template, undefined, tableName, JSON.stringify(adaptive.scanResult));
            return { periodId: period.id, taskCount: period.tasks.length, reasoning: adaptive.reasoning };
        });
        if (!periodCreateRes.ok) {
            log.fail(periodCreateRes.ms, periodCreateRes.error);
            recordStep("createPeriod", "fail", periodCreateRes.ms, undefined, periodCreateRes.error);
            return report;
        }
        ...
        const reconRes = await timeStep("reconcile", async () => {
            ...
            for (const recon of allRecons) {
                try {
                    ...
                    results.errored++;
                    errors.push(`${recon.name}: ${e.message}`);
                }
            }
            return { results, errors };
        });
        if (!reconRes.ok) {
            log.fail(reconRes.ms, reconRes.error);
            recordStep("reconcile", "fail", reconRes.ms, undefined, reconRes.error);
        }
        else {
            ...
            if (r.errored > 0)
                log.warn(reconRes.ms, detail);
            else
                log.pass(reconRes.ms, detail);
        ...
        if (!fluxRes.ok) {
            log.fail(fluxRes.ms, fluxRes.error);
            recordStep("flux", "fail", fluxRes.ms, undefined, fluxRes.error);
        }
        else {
        ...
        if (!plRes.ok) {
            log.fail(plRes.ms, plRes.error);
            recordStep("plReview", "fail", plRes.ms, undefined, plRes.error);
        }
        else {
        ...
        if (!readyRes.ok) {
            log.fail(readyRes.ms, readyRes.error);
            recordStep("readiness", "fail", readyRes.ms, undefined, readyRes.error);
        }
        else {
...
async function main() {
    ...
    for (const r of allReports) {
        ...
        for (const s of issues) {
            ...
            if (s.error)
                console.log(`      ${s.error.slice(0, 200)}`);
            issueCount++;
        }
    }
    if (issueCount === 0) {
