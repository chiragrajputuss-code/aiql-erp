 ...
 */
function getFiscalYear(startYear, fiscalStartMonth) {
    return {
        fyStart: startOfMonth(startYear, fiscalStartMonth),
        fyEnd: endOfMonth(startYear + 1, fiscalStartMonth - 1 || 12),
    };
}
function currentFiscalYear(config, today = new Date()) {
...
function fiscalQuarterRange(quarter, fyStartYear, fyStartMonth) {
...
function currentFiscalQuarter(config, today = new Date()) {
    const { fyStart } = currentFiscalYear(config, today);
    const monthsSinceFyStart = (today.getFullYear() - fyStart.getFullYear()) * 12 +
 ...
 *  - "YTD" / "year to date"
 */
export function parsePeriod(text, config, today = new Date()) {
