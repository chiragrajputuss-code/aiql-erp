...
describe("Organisation CRUD", () => {
    ...
    it("enforces unique slugs", async () => {
        ...
        await expect(prisma.organisation.create({ data: { name: "Org B", slug, queriesResetAt: new Date() } })).rejects.toThrow();
    });
...
describe("User creation with org relation", () => {
    ...
    it("enforces unique email", async () => {
        ...
        await expect(prisma.user.create({ data: { email: `${slug}@test.io`, role: "MEMBER", orgId: org.id } })).rejects.toThrow();
    });
});
// ── Seed ─────────────────────────────────────────────────────────────────────
describe("Seed data", () => {
