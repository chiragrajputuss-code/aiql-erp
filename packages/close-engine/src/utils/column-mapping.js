 ...
 */
export async function buildColMap(connectionId) {
    ...
    const mappings = JSON.parse(file.columnMapping);
 ...
 */
export function applyColMap(sql, colMap) {
    let result = sql;
    for (const [alias, canonical] of colMap) {
        ...
        result = result.replace(new RegExp(`(?<!::)(?<![\\w.])${alias}\\b`, "g"), canonical);
    }
...
export async function getTableColumns(tableName) {
    const rows = await prisma.$queryRawUnsafe(`SELECT column_name FROM information_schema.columns
 ...
 */
export function makeSqlDefensive(sql, presentColumns) {
    let result = sql;
...
export async function getTableName(connectionId) {
...
export async function loadAccountTypeMap(connectionId) {
    const map = new Map();
    ...
    if (conn?.schemaCacheJson) {
        try {
            const parsed = JSON.parse(conn.schemaCacheJson);
    ...
    for (const m of saved)
        map.set(m.accountName, m.accountType);
    return map;
}
export function accountsByType(typeMap) {
    const result = { bank: [], payable: [], receivable: [], tax: [], inventory: [] };
