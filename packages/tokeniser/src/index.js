 ...
 */
export function tokenise(text, config = {}, dictionary) {
    const t0 = Date.now();
    ...
    if (cfg.tokeniseAmounts) {
        ...
        for (const amt of amountsSorted) {
            if (map.getToken(amt.originalText))
                continue;
    ...
    for (const term of cfg.customStripList ?? []) {
        if (term.trim())
            working = working.split(term.trim()).join("");
    }
    return {
        original: text,
        tokenised: working,
