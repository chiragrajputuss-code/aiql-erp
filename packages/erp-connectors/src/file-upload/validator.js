...
export function validateMappings(mappings) {
    ...
    const canonical = active.map((m) => m.canonicalName);
    ...
    if (!hasDate)
        errors.push("No date column found — at least one of: transaction_date, due_date, value_date is required");
    if (!hasAccount)
        errors.push("No account identifier found — at least one of: account_name, account_code, party_name, vendor_name, customer_name is required");
    if (!hasAmount)
        errors.push("No amount column found — at least one of: debit_amount, credit_amount, net_amount is required");
    // ── Column count guards ───────────────────────────────────────────────────
    if (active.length < 3) {
