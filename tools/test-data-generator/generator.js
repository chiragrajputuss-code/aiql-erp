 ...
 */
import { buildChartOfAccounts } from "./companies";
...
const intBetween = (min, max, rng) => Math.floor(rng() * (max - min + 1)) + min;
const between = (min, max, rng) => rng() * (max - min) + min;
...
// ─── Date helpers ────────────────────────────────────────────────────────────
function dateInRange(start, end, rng) {
...
function genSales(ctx, date, accounts) {
    const customer = pick(ctx.customers, ctx.rng);
    const revenueAcct = pick(ctx.revenues, ctx.rng);
    const baseAmount = round2(between(2000, 200000, ctx.rng));
...
function genPurchase(ctx, date) {
    const vendor = pick(ctx.vendors, ctx.rng);
    const cogsAcct = pick(ctx.cogs, ctx.rng);
    const baseAmount = round2(between(5000, 300000, ctx.rng));
...
function genReceipt(ctx, date) {
    const customer = pick(ctx.customers, ctx.rng);
    const bank = pick(ctx.bankAccounts, ctx.rng);
    const amount = round2(between(2000, 250000, ctx.rng));
...
function genPayment(ctx, date) {
    const vendor = pick(ctx.vendors, ctx.rng);
    const bank = pick(ctx.bankAccounts, ctx.rng);
    const amount = round2(between(5000, 300000, ctx.rng));
...
function genJournal(ctx, date) {
    const expense = pick(ctx.expenses, ctx.rng);
    const bank = pick(ctx.bankAccounts, ctx.rng);
    const amount = round2(between(500, 100000, ctx.rng));
...
function genContra(ctx, date) {
    if (ctx.bankAccounts.length < 2)
        return genJournal(ctx, date);
    const [bank1, bank2] = [...ctx.bankAccounts].sort(() => ctx.rng() - 0.5);
    const amount = round2(between(10000, 500000, ctx.rng));
...
export function generateTransactions(company, seed) {
    ...
    const vendors = [];
    const customers = [];
    for (let i = 0; i < company.vendorCount; i++)
        vendors.push(generateVendorName(rng));
    for (let i = 0; i < company.customerCount; i++)
        customers.push(generateCustomerName(rng));
    const ctx = {
    ...
    if (ctx.bankAccounts.length === 0)
        ctx.bankAccounts = company.bankAccounts.slice(0, 1);
    if (ctx.expenses.length === 0)
        ctx.expenses = ["Office Rent"];
    if (ctx.revenues.length === 0)
        ctx.revenues = ["Service Income"];
    if (ctx.cogs.length === 0)
        ctx.cogs = ctx.expenses;
    ...
    while (transactions.length < targetCount) {
        ...
        switch (picked) {
            case "Sales":
                lines = genSales(ctx, date, accounts);
                break;
            case "Purchase":
                lines = genPurchase(ctx, date);
                break;
            case "Receipt":
                lines = genReceipt(ctx, date);
                break;
            case "Payment":
                lines = genPayment(ctx, date);
                break;
            case "Contra":
                lines = genContra(ctx, date);
                break;
...
function applySeededIssues(txns, company, rng, log) {
    ...
    for (let i = 0; i < cfg.voucherImbalance; i++) {
        ...
        if (lastLine.debit > 0)
            lastLine.debit = round2(lastLine.debit + delta);
        else
            lastLine.credit = round2(lastLine.credit + delta);
        log.push({
    ...
    for (let i = 0; i < cfg.duplicateTransactions; i++) {
        ...
        if (original.debit === 0 && original.credit === 0)
            continue;
    ...
    for (let i = 0; i < cfg.unclassifiedAccounts; i++) {
        ...
        const amount = round2(between(100, 10000, rng));
        const acct = pick(weirdAccounts, rng);
        const vNo = `JV-X${String(intBetween(100, 999, rng))}`;
        const dt = isoDateOnly(date);
        txns.push({ date: dt, voucherNo: vNo, voucherType: "Journal", accountName: acct,
            debit: amount, credit: 0, narration: `Unknown adjustment` }, { date: dt, voucherNo: vNo, voucherType: "Journal", accountName: company.bankAccounts[0],
    ...
    if (company.gstRegime === "regular" && cfg.gstMismatch > 0) {
        const salesVouchers = Array.from(new Set(txns.filter((t) => t.voucherType === "Sales").map((t) => t.voucherNo)));
    ...
    for (let i = 0; i < cfg.signAnomalies; i++) {
        ...
        const date = dateInRange(start, new Date(company.periodEnd), rng);
        const amount = round2(between(50000, 200000, rng));
        const vNo = `JV-S${String(intBetween(100, 999, rng))}`;
        const dt = isoDateOnly(date);
        // Put Dr in Sundry Creditors (reverse direction)
        txns.push({ date: dt, voucherNo: vNo, voucherType: "Journal", accountName: "Sundry Creditors",
            partyName: vendor, vendorName: vendor, debit: amount, credit: 0,
            narration: `Refund/advance from vendor` }, { date: dt, voucherNo: vNo, voucherType: "Journal", accountName: company.bankAccounts[0],
