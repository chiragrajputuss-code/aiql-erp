...
async function loadAccountTypeIndex(connectionId) {
    const connection = await prisma.erpConnection.findUnique({
        where: { id: connectionId },
        select: { schemaCacheJson: true },
    });
    const idx = { bank: [], payable: [], receivable: [], tax: [], inventory: [] };
    if (!connection?.schemaCacheJson)
        return idx;
    try {
        const schema = JSON.parse(connection.schemaCacheJson);
        const map = schema.accountTypeMap ?? {};
        for (const [name, type] of Object.entries(map)) {
            if (type === "BANK" || type === "CASH")
                idx.bank.push(name);
            else if (type === "PAYABLE" || type === "CURRENT_LIABILITY")
                idx.payable.push(name);
            else if (type === "RECEIVABLE")
                idx.receivable.push(name);
            else if (type === "TAX")
                idx.tax.push(name);
            else if (type === "INVENTORY")
                idx.inventory.push(name);
        }
    }
...
function accountFilter(colName, accounts, fallbackLike) {
 ...
 */
function applyAccountTypeIndex(sql, idx) {
...
async function runSql(sql, params = []) {
    for (const re of WRITE_PATTERNS) {
        if (re.test(sql))
            throw new Error("Write operations are not permitted in reconciliation queries");
    }
    ...
    const rows = params.length > 0
        ? await prisma.$queryRawUnsafe(sql.trim(), ...params)
        : await prisma.$queryRawUnsafe(sql.trim());
...
async function analyzeVariance(reconName, sourceBalance, targetBalance, variance, threshold, items, startDate, endDate) {
    ...
    const userPrompt = `Reconciliation: "${reconName}"\n` +
    ...
    try {
        ...
        return {
            ...
            findings: parsed.findings.filter((f) => !!f && typeof f.type === "string" && typeof f.description === "string"),
...
// ─── Core runner ──────────────────────────────────────────────────────────────
export async function runReconciliation(reconId) {
    ...
    const [colMap, accountIdx, presentColumns] = await Promise.all([
        ...
        tableName ? getTableColumns(tableName) : Promise.resolve(new Set()),
    ]);
    const applyMap = (sql) => {
    ...
    try {
        ...
        if (!passed) {
            ...
            const analysis = await analyzeVariance(recon.name, sourceBalance, targetBalance, variance, recon.varianceThreshold, varianceItems, recon.task.period.startDate, recon.task.period.endDate);
...
export async function runAllReconciliations(periodId) {
...
// ─── Detail query ─────────────────────────────────────────────────────────────
export async function getReconciliationDetail(reconId, cachedItems) {
    const recon = await prisma.reconciliation.findUniqueOrThrow({
        where: { id: reconId },
    });
