...
import { buildVoucherImbalanceIssue, buildDuplicateIssue, compareIssues, isMaterialFlux, toNum, isoDateOnly, getPriorPeriod, bucketAccountsByType, estimateCostInr, QUESTION_BUDGET_MAX, QUESTION_MATERIALITY_INR, FLUX_MATERIAL_ABS_THRESHOLD, } from "../lib/issue-detection";
describe("buildVoucherImbalanceIssue", () => {
    ...
    it("handles BigInt-like number coercion", () => {
        const issue = buildVoucherImbalanceIssue([
            { reference_number: "VCH-001", dr: 100, cr: 50, diff: 50 },
        ]);
...
describe("compareIssues", () => {
    const critical = { code: "a", severity: "critical", category: "", title: "", description: "", affectedRows: 1, exposure: 100, examples: [] };
    const review = { code: "b", severity: "review", category: "", title: "", description: "", affectedRows: 1, exposure: 1000, examples: [] };
    const info = { code: "c", severity: "info", category: "", title: "", description: "", affectedRows: 1, exposure: 10000, examples: [] };
...
describe("isMaterialFlux", () => {
    it("not material if absolute variance below threshold", () => {
        expect(isMaterialFlux(40000, 100000)).toBe(false);
    });
    it("not material if variance above abs threshold but below 10% of prior", () => {
        expect(isMaterialFlux(60000, 10000000)).toBe(false); // only 0.6%
    });
    it("material when both abs threshold and 10% threshold are exceeded", () => {
        expect(isMaterialFlux(60000, 200000)).toBe(true); // 30%, ₹60K
    });
    it("material when prior is zero and current exceeds abs threshold", () => {
        expect(isMaterialFlux(60000, 0)).toBe(true);
        expect(isMaterialFlux(40000, 0)).toBe(false);
    });
    it("uses the FLUX_MATERIAL_ABS_THRESHOLD constant correctly", () => {
        expect(isMaterialFlux(FLUX_MATERIAL_ABS_THRESHOLD - 1, 100000)).toBe(false);
        expect(isMaterialFlux(FLUX_MATERIAL_ABS_THRESHOLD, 100000)).toBe(true);
    });
    it("treats negative variance the same as positive", () => {
        expect(isMaterialFlux(-60000, 200000)).toBe(true);
    });
});
describe("toNum", () => {
...
describe("getPriorPeriod", () => {
    it("computes prior period as same length, immediately before", () => {
        const { start, end } = getPriorPeriod(new Date("2026-04-01"), new Date("2026-04-30"));
        expect(isoDateOnly(end)).toBe("2026-03-31");
        expect(isoDateOnly(start)).toBe("2026-03-02"); // 30 days back
    });
    it("handles single-day period", () => {
        const { start, end } = getPriorPeriod(new Date("2026-04-15"), new Date("2026-04-15"));
...
describe("estimateCostInr", () => {
    it("returns 0 for free Groq model", () => {
        expect(estimateCostInr("groq:llama-3.1-8b-instant", 10000, 5000)).toBe(0);
    });
    it("calculates cost for paid Groq model", () => {
        // 10K input × $0.59/M = $0.0059
        // 5K output × $0.79/M = $0.00395
        // Total ≈ $0.00985 × 83 ≈ ₹0.82
        const cost = estimateCostInr("groq:llama-3.3-70b-versatile", 10000, 5000);
    ...
    it("calculates cost for Claude Haiku", () => {
        ...
        const cost = estimateCostInr("anthropic:claude-haiku-4-5", 10000, 5000);
        expect(cost).toBeGreaterThan(2);
        expect(cost).toBeLessThan(3);
    });
    it("falls back to default rates for unknown provider", () => {
        const cost = estimateCostInr("unknown:model", 1000, 500);
        expect(cost).toBeGreaterThan(0);
    });
});
describe("budget constants", () => {
    it("has expected materiality threshold", () => {
        expect(QUESTION_MATERIALITY_INR).toBe(10000);
    });
