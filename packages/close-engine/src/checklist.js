...
// ─── Period creation ──────────────────────────────────────────────────────────
export async function createClosePeriodFromTemplate(orgId, connectionId, connectionIds, periodName, startDate, endDate, template, targetCompletionDate, tableName, scanResultJson, meta) {
    const periodId = await prisma.$transaction(async (tx) => {
        ...
        const createdTasks = await Promise.all(template.tasks.map((t) => tx.closeTask.create({
            data: {
                periodId: period.id,
                title: t.title,
                category: t.category,
        ...
        const keyToId = new Map(template.tasks.map((t, i) => [t.key, createdTasks[i].id]));
        // 4. Patch each task with resolved dependency IDs
        await Promise.all(template.tasks.map((t, i) => {
...
export async function resolveDependencies(periodId) {
    ...
    while (changed && iterations < 10) {
        ...
        for (const task of tasks) {
            ...
            const allDepsDone = task.dependsOnIds.every((id) => statusById.get(id) === "COMPLETED");
            let next = current;
    ...
    // Persist all changes
    await Promise.all(Array.from(updates.entries()).map(([id, status]) => prisma.closeTask.update({ where: { id }, data: { status: status } })));
}
// ─── Task status update ───────────────────────────────────────────────────────
export async function updateTaskStatus(taskId, newStatus, notes) {
...
// ─── Progress ─────────────────────────────────────────────────────────────────
export async function recalculateProgress(periodId) {
    const tasks = await prisma.closeTask.findMany({ where: { periodId } });
...
export async function calculateProgress(periodId) {
    const tasks = await prisma.closeTask.findMany({ where: { periodId } });
    const total = tasks.length;
    const completed = tasks.filter((t) => t.status === "COMPLETED").length;
...
// ─── Blockers ─────────────────────────────────────────────────────────────────
export async function getBlockers(periodId) {
...
// ─── Queries ──────────────────────────────────────────────────────────────────
export async function getPeriodWithTasks(periodId) {
...
export async function getTaskWithRecon(taskId) {
