 ...
 */
export function sanitiseWatchAccount(raw) {
    if (typeof raw !== "string")
        return null;
    const trimmed = raw.trim();
    if (trimmed.length === 0 || trimmed.length > MAX_WATCH_LEN)
        return null;
    if (!SAFE_WATCH_ACCOUNT_RE.test(trimmed))
        return null;
...
export async function parseUserIntent(userText, ctx = {}) {
    const t0 = Date.now();
...
function looksLikeAccountName(fragment) {
...
function heuristicParse(text) {
    ...
    for (const { re, key } of EXCLUSION_PATTERNS) {
        if (re.test(text) && !exclusions.includes(key))
            exclusions.push(key);
    }
    ...
    const partyHintRe = /(?:check|verify|deep[- ]?dive|review|look at|investigate)\s+([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z&.]+){0,3}(?:\s+(?:Pvt|Ltd|LLP|Inc|Co|Bros|Group|Industries|Enterprises|Traders|Agencies|Services|Works|Mills|Suppliers)\.?)?)/g;
    let m;
    ...
    const matchesEverything = (focusAreas.length > 0 || exclusions.length > 0 || watchAccounts.length > 0 || watchParties.length > 0) &&
...
function summarise(p) {
    ...
    if (p.focusAreas.length > 0)
        parts.push(`focus on ${p.focusAreas.join(", ")}`);
    if (p.watchAccounts.length > 0)
        parts.push(`watch ${p.watchAccounts.length} account(s)`);
    if ((p.watchParties ?? []).length > 0)
        parts.push(`deep-dive ${p.watchParties.length} party/vendor(s)`);
    if (p.exclusions.length > 0)
        parts.push(`skip ${p.exclusions.join(", ")}`);
...
async function llmParse(text, ctx) {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey)
        return null;
    const ctxLines = [];
    if (ctx.accountSummary) {
        ctxLines.push(`Available data: ${ctx.accountSummary.bank} bank, ${ctx.accountSummary.ar} AR, ` +
    ...
    const userPrompt = (ctxLines.length > 0 ? `Context:\n${ctxLines.join("\n")}\n\n` : "") +
        `User instruction:\n${text}\n\nReturn JSON only.`;
...
function normaliseLlmOutput(raw) {
    let parsed;
    try {
    ...
    const strArr = (k, max, dedupeLower = false) => {
        const v = parsed[k];
        if (!Array.isArray(v))
            return [];
        const out = [];
    ...
    const focusAreas = strArr("focusAreas", 8, true).filter((s) => ALLOWED_FOCUS.has(s));
    const exclusions = strArr("exclusions", 8, true).filter((s) => ALLOWED_EXCLUSIONS.has(s));
