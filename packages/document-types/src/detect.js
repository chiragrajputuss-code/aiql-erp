...
function scoreType(uploadedNorm, def) {
    ...
    const antiMatches = detection.antiSignalColumns.filter((c) => uploadedNorm.has(normalise(c)));
    ...
    const missingReq = detection.requiredColumns.filter((c) => !uploadedNorm.has(normalise(c)));
...
// ─── Main export ──────────────────────────────────────────────────────────────
export function detectDocumentType(columns, sheetName) {
...
export function detectMultiSheet(sheets) {
