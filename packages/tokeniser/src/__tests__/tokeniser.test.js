...
const dict = (vendors = [], customers = [], employees = []) => ({ vendors, customers, employees });
// ─── Full pipeline ────────────────────────────────────────────────────────────
describe("full pipeline", () => {
    it("tokenises vendors from dictionary", () => {
        const { tokenised, tokenMap } = tokenise("Show AP aging for Sharma Enterprises", {}, dict(["Sharma Enterprises"]));
    ...
    it("strips PII first — PII does not appear in tokenised output", () => {
        const { tokenised, stats } = tokenise("Employee PAN ABCDE1234F has outstanding balance of ₹50,000", {}, dict());
    ...
    it("handles text with vendors + amounts + PII together", () => {
        const text = "Vendor Acme Corp owes ₹1Cr. Employee SSN 123-45-6789 on file. Account 4000-100 impacted.";
    ...
    it("tokenises customer separately from vendor", () => {
        const { tokenised } = tokenise("Vendor Acme Corp and customer Infosys Ltd both have dues", {}, dict(["Acme Corp"], ["Infosys Ltd"]));
        expect(tokenised).toContain("VENDOR_T001");
        expect(tokenised).toContain("CUSTOMER_T001");
    });
    it("returns correct stats", () => {
        const { stats } = tokenise("Acme Corp owes ₹5L. Balance 4000 account.", {}, dict(["Acme Corp"]));
...
describe("config flags", () => {
    ...
    it("customStripList removes terms from output", () => {
        const { tokenised } = tokenise("confidential note: INTERNAL DO NOT SHARE — balance 5000", { customStripList: ["INTERNAL DO NOT SHARE"] }, dict());
...
describe("round-trip: tokenise → detokenise", () => {
    ...
    it("multiple tokens round-trip correctly", () => {
        const sql = "SELECT * FROM ap WHERE vendor = 'Acme Corp' AND customer = 'Infosys Ltd' AND amount > ₹10L";
        const { tokenised, tokenMap } = tokenise(sql, {}, dict(["Acme Corp"], ["Infosys Ltd"]));
        const restored = detokenise(tokenised, tokenMap);
        expect(restored).toContain("Acme Corp");
    ...
    it("tokenised output contains NO original sensitive values", () => {
        const { tokenised } = tokenise("Vendor Tata Motors owes ₹5L on account 4000", {}, dict(["Tata Motors"]));
...
describe("previewTokenisation", () => {
    it("returns original and tokenised text", () => {
        const { original, tokenised } = previewTokenisation("Invoice from Acme Corp for ₹5L", {}, dict(["Acme Corp"]));
    ...
    it("tokens are sorted by position", () => {
        const { tokens } = previewTokenisation("Acme Corp owes ₹5L, Infosys Ltd owes ₹10L", {}, dict(["Acme Corp"], ["Infosys Ltd"]));
...
describe("performance", () => {
    const TYPICAL_QUERY = "Show me AP aging for Acme Corp, Sharma Enterprises, and Tata Motors where " +
    ...
    it("tokenises a typical query in <50ms", () => {
        const { stats } = tokenise(TYPICAL_QUERY, {}, dict(["Acme Corp", "Sharma Enterprises", "Tata Motors"], ["Infosys Ltd"]));
