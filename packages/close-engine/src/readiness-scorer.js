...
// ─── Dimension scorers ────────────────────────────────────────────────────────
export function scoreDataIntegrity(scan) {
    let score = 100;
    const issues = [];
    if (!scan) {
        return {
    ...
    if (critical > 0)
        issues.push(`${critical} critical data integrity issue${critical > 1 ? "s" : ""}`);
    // Review-level lighter (max -15)
    const review = scan.bySeverity.review ?? 0;
    score -= Math.min(15, review * 3);
    if (review > 0)
        issues.push(`${review} review-level issue${review > 1 ? "s" : ""}`);
    return {
        name: "Data Integrity",
...
export function scorePeriodScope(scan) {
    let score = 100;
    const issues = [];
    if (!scan) {
        return {
...
export function scoreReconciliation(reconStats) {
    const issues = [];
    ...
    if (reconStats.failed > 0)
        issues.push(`${reconStats.failed} reconciliation${reconStats.failed > 1 ? "s" : ""} failed`);
    if (reconStats.pending > 0)
        issues.push(`${reconStats.pending} reconciliation${reconStats.pending > 1 ? "s" : ""} not yet run`);
    return {
        name: "Reconciliation",
        score,
        weight: DIMENSION_WEIGHTS.reconciliation,
...
export function scoreCompliance(scan) {
    let score = 100;
    const issues = [];
    if (!scan) {
        return {
...
export function scoreVarianceAnalysis(fluxStats) {
    const issues = [];
    let score = 100;
    if (!fluxStats.run) {
        return {
    ...
    if (unexplained > 0)
        issues.push(`${unexplained} material variance${unexplained > 1 ? "s" : ""} without AI explanation`);
    return {
        name: "Variance Analysis",
        score: clamp(score, 0, 100),
        weight: DIMENSION_WEIGHTS.varianceAnalysis,
...
export function scoreWorkflowProgress(taskStats) {
    const issues = [];
    ...
    if (taskStats.failed > 0)
        issues.push(`${taskStats.failed} task${taskStats.failed > 1 ? "s" : ""} failed`);
    if (taskStats.blocked > 0)
        issues.push(`${taskStats.blocked} task${taskStats.blocked > 1 ? "s" : ""} blocked`);
    return {
        name: "Workflow Progress",
        score,
        weight: DIMENSION_WEIGHTS.workflowProgress,
...
// ─── Hard gates ───────────────────────────────────────────────────────────────
export function evaluateHardGates(scan, taskCount) {
    const gates = [];
...
function buildTopActions(hardGates, dimensions, reconStats, fluxRun) {
    const actions = [];
    // Failed hard gates first
    for (const gate of hardGates) {
        if (!gate.passed)
            actions.push(`🛑 Fix: ${gate.message}`);
    }
...
// ─── Main entry point ────────────────────────────────────────────────────────
export async function computeReadinessScore(periodId) {
    ...
    if (period.scanResultJson) {
        try {
            scan = JSON.parse(period.scanResultJson);
        }
        catch {
            scan = null;
        }
    ...
    for (const fr of fluxRuns) {
        ...
        try {
            const result = JSON.parse(fr.resultJson);
    ...
    if (!allGatesPassed)
        status = "blocked";
    else if (totalScore >= READY_THRESHOLD)
        status = "ready";
