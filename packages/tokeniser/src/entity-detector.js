...
function findPosition(text, value) {
...
const STOPWORDS_LOWER = new Set(Array.from(ACCOUNTING_STOPWORDS).map((w) => w.toLowerCase()));
function isStopword(value) {
    return STOPWORDS_LOWER.has(value.trim().toLowerCase());
}
// ─── Pass 1: Dictionary Match ─────────────────────────────────────────────────
function pass1Dictionary(text, dictionary) {
    const results = [];
    const categoryMap = [
...
// ─── Pass 2: NLP NER ──────────────────────────────────────────────────────────
function pass2NLP(text, alreadyFound) {
    const results = [];
    const doc = nlp(text);
    const nlpMatches = [];
    const orgs = doc.organizations().json({ offset: true });
    for (const m of orgs) {
        nlpMatches.push({ value: m.text.trim(), category: "ENTITY" });
    }
    const people = doc.people().json({ offset: true });
    ...
    for (const { value, category } of nlpMatches) {
        ...
        const alreadyCovered = alreadyFound.some((e) => e.value.toLowerCase() === value.toLowerCase() || overlaps(e.position, pos));
        if (alreadyCovered)
            continue;
...
function pass3Context(text, alreadyFound) {
    const results = [];
