...
export class ZohoBooksConnector {
    constructor(credentials, onTokenRefresh) {
        this.erpType = "ZOHO_BOOKS";
    ...
    async refreshAccessToken() {
        const res = await axios.post(`${this.accountsBase}/oauth/v2/token`, new URLSearchParams({
    ...
    async testConnection() {
        ...
        try {
            ...
            return { success: false, message: `Zoho connection error: ${err.message}` };
        }
    }
    // ── Delegated methods ─────────────────────────────────────────────────────
...
export function buildAuthorizationUrl(clientId, redirectUri, state, region = "IN") {
...
export async function exchangeCodeForTokens(code, clientId, clientSecret, redirectUri, region = "IN") {
    const base = ZOHO_REGIONS[region].accounts;
    const res = await axios.post(`${base}/oauth/v2/token`, new URLSearchParams({
