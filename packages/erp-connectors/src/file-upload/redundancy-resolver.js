 ...
 */
export function resolveRedundancy(mappings, fileType = "transaction") {
    const resolved = mappings.map((m) => ({ ...m, dropped: false }));
    const canonical = (name) => resolved.find((m) => !m.dropped && m.canonicalName === name);
    ...
    if (canonical("vendor_name") && canonical("party_name")) {
        ...
        if (v) {
            v.dropped = true;
            v.dropReason = "redundant: party_name is more generic";
        }
    }
    // R3: customer_name + party_name → drop customer_name
    if (canonical("customer_name") && canonical("party_name")) {
        ...
        if (c) {
            c.dropped = true;
            c.dropReason = "redundant: party_name is more generic";
        }
    }
    ...
    if (fileType === "transaction") {
        ...
        if (ob) {
            ob.dropped = true;
            ob.dropReason = "opening_balance not expected in transaction file";
        }
    }
    // R5: duplicate semantic type → keep first, drop rest
    const seenCanonical = new Map();
    for (const m of resolved) {
        if (!m.canonicalName || m.dropped)
            continue;
