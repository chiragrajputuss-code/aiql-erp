 ...
 */
export function classifyAccounts(accounts, knowledge) {
    const result = {};
    for (const account of accounts) {
        const groupKey = account.group.trim().toLowerCase();
        const type = knowledge.accountGroups[groupKey] ?? "UNKNOWN";
 ...
 */
export function classifyByName(accountName) {
 ...
 */
export function fillUnknownsByName(map) {
    const result = { ...map };
 ...
 */
export function classifyAccountNames(names) {
    const result = {};
    for (const name of names)
        result[name] = classifyByName(name);
    return result;
}
 ...
 */
export async function classifyByLLM(names) {
    if (names.length === 0)
        return {};
    ...
    for (let i = 0; i < names.length; i += BATCH_SIZE) {
        ...
        if (!safeRes || !safeRes.content)
            continue;
        try {
            const parsed = JSON.parse(safeRes.content);
            for (const c of parsed.classifications ?? []) {
                if (!c.name || !c.type)
                    continue;
                const validType = (["BANK", "CASH", "RECEIVABLE", "PAYABLE", "TAX", "INVENTORY",
                    "FIXED_ASSET", "CURRENT_ASSET", "CURRENT_LIABILITY", "REVENUE",
                    "OTHER_INCOME", "EXPENSE", "COGS", "EQUITY", "INVESTMENT", "UNKNOWN"]
                    .includes(c.type)) ? c.type : "UNKNOWN";
 ...
 */
export async function cascadeClassify(names, existingMap = {}) {
    const map = {};
    const sources = {};
    const llmConfidences = {};
 ...
 */
export function groupByType(accountTypeMap) {
    const grouped = {};
