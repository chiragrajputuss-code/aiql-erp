...
describe("createConnector factory", () => {
    ...
    it("throws for unknown ERP type", () => {
        expect(() => createConnector("CUSTOM", {})).toThrow("Unsupported ERP type");
    });
});
// ─── TallyConnector ───────────────────────────────────────────────────────────
describe("TallyConnector — construction", () => {
    it("uses provided host and port", () => {
        ...
        expect(c.baseUrl).toBe("http://10.0.0.5:9001");
    });
...
describe("testConnection()", () => {
    ...
    it("sends a POST to the correct URL", async () => {
        ...
        expect(mockPost).toHaveBeenCalledWith("http://tally.mycompany.com:9000", expect.stringContaining("<TALLYREQUEST>"), expect.objectContaining({ headers: expect.objectContaining({ "Content-Type": expect.stringContaining("xml") }) }));
    });
    it("XML request contains correct Export Data structure", async () => {
        mockPost.mockResolvedValueOnce({ data: LEDGER_XML });
        const c = new TallyConnector({ host: "192.168.1.10" });
        await c.testConnection();
        const sentXml = mockPost.mock.calls[0][1];
...
describe("introspectSchema()", () => {
    ...
    it("ledger→group relationship is present", async () => {
        ...
        const rel = relationships.find((r) => r.fromTable === "ledgers" && r.toTable === "groups");
