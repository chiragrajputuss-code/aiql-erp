...
function partyCol(cols, prefer) {
    if (prefer === "vendor") {
        if (cols.has("vendor_name"))
            return "vendor_name";
        if (cols.has("party_name"))
            return "party_name";
        return "account_name";
    }
...
function debitCol(cols) {
    if (cols.has("debit_amount"))
        return "debit_amount";
...
const TEMPLATES = [
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            const vtype = cols.has("voucher_type") ? "voucher_type"
                : cols.has("transaction_type") ? "transaction_type" : null;
            const desc = cols.has("description") ? "description" : "''";
            const salaryFilter = `(LOWER(COALESCE(${acct}, '')) LIKE ANY(ARRAY['%salary%','%payroll%','%wages%','%remuneration%','%staff%'])` +
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            const tdsFilter = `(LOWER(COALESCE(${acct},'')) LIKE '%tds%'` +
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            const vtype = cols.has("voucher_type") ? "voucher_type"
                : cols.has("transaction_type") ? "transaction_type" : null;
            const advFilter = `(LOWER(COALESCE(${acct},'')) LIKE '%advance%'` +
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            if (cols.has("closing_balance")) {
                return (`SELECT account_name, ROUND(MAX(closing_balance), 2) AS balance` +
                    ` FROM "${t}"` +
            ...
            return (`SELECT account_name${grp},` +
                ` ROUND(SUM(COALESCE(${dr}, 0)) - SUM(COALESCE(${cr}, 0)), 2) AS balance` +
                ` FROM "${t}"` +
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            const grp = cols.has("account_group") ? "account_group"
                : cols.has("account_type") ? "account_type" : null;
            const acct = cols.has("account_name") ? "account_name" : "NULL";
            const assetFilter = `(LOWER(COALESCE(${grp ? grp : acct},'')) LIKE ANY(ARRAY['%fixed asset%','%capital work%','%plant%','%equipment%','%machinery%','%furniture%','%vehicle%','%building%','%land%']))`;
            return (`SELECT` +
                `  ${acct} AS asset,` +
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            return (`SELECT COALESCE(${vendor}, 'Unknown') AS vendor,` +
    ...
    },
    {
        ...
        buildSql: (t, cols) => {
            ...
            return (`SELECT COALESCE(${customer}, 'Unknown') AS customer,` +
...
export function getSqlForTemplate(templateId, schema) {
...
export function matchTemplate(question, schema) {
    const primaryTable = schema.tables[0];
    if (!primaryTable)
        return null;
